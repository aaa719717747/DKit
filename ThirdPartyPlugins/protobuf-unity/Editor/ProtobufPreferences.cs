...
namespace Im.Unity.protobuf_unity.Editor
{
    public static class ProtobufPreferences
    {
        ...
        {
            get { return EditorPrefs.GetBool(prefLogError, true); }
            set { EditorPrefs.SetBool(prefLogError, value); }
        }

        internal static bool LogDebug
        {
            get { return EditorPrefs.GetBool(prefLogDebug, false); }
            set { EditorPrefs.SetBool(prefLogDebug, value); }
        }

        internal class ProtobufUnitySettingsProvider : SettingsProvider
        {
            public ProtobufUnitySettingsProvider(string path, SettingsScope scope = SettingsScope.User) : base(path,
                scope)
            {
            }
        ...
        private static void ProtobufPreference()
        {
            ...
            LogErrors = EditorGUILayout.Toggle(
                new GUIContent("Log Error Output", "Log compilation errors from protoc command."), LogErrors);
            LogDebug = EditorGUILayout.Toggle(
                new GUIContent("Log Standard Output", "Log compilation completion messages."), LogDebug);
